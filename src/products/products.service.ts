...
export class ProductsService {
    ...
    public async saveProduct(user: UsersDTO, productData: ProductsDTO, dealData: DealsDTO): Promise<CommonResponseModel> {
        if (user.role !== 'Admin') {
    ...
    public async homePage(query:any): Promise<CommonResponseModel> {
        ...
        let products = await this.productModel.find({status: 1},'title filePath imageUrl isDealAvailable delaPercent category variant averageRating').limit(limit).sort({createdAt:-1});
        ...
        const categories = await this.categoryModel.find({status: 1},'title filePath imageUrl category isSubCategoryAvailable').limit(catLimit).sort({createdAt:-1});
        const dealsOfDay = await this.dealsModel.find({status: 1},'name filePath delaPercent imageUrl delalType category product').limit(topLimit).sort({createdAt:+1});
        const topDeals = await this.dealsModel.find({status: 1,topDeal:true},'name filePath imageUrl delaPercent delalType category product').limit(topLimit).sort({createdAt:-1});
        return {
            response_code: HttpStatus.OK,
    ...
    public async homePageTopDeal(): Promise<CommonResponseModel> {
        const topDeals = await this.dealsModel.find({status: 1,topDeal:true},'name imageUrl filePath delaPercent delalType category product').sort({createdAt:-1});
        return {
            response_code: HttpStatus.OK,
    ...
    public async homePageDealsOfDay(): Promise<CommonResponseModel> {
        const topDeals = await this.dealsModel.find({status: 1},'name filePath imageUrl delaPercent delalType category product').sort({createdAt:-1});
        return {
            response_code: HttpStatus.OK,
            response_data: topDeals
        };
    ...
    public async homePageCategory(): Promise<CommonResponseModel> {
        const categories = await this.categoryModel.find({status: 1},'title filePath imageUrl category').sort({createdAt:-1});
        return {
            response_code: HttpStatus.OK,
    ...
    public async homePageProduct(query:any): Promise<CommonResponseModel> {
        let products;
        if(query && query.page){
            products = await this.productModel.find({status: 1},'title filePath imageUrl category isDealAvailable delaPercent variant averageRating').skip(Number(query.page)).limit(8).sort({createdAt:-1});

        }else{
            products = await this.productModel.find({status: 1},'title filePath imageUrl category isDealAvailable delaPercent variant averageRating').sort({createdAt:-1});
        }
        if(query && query.userId && products && products.length){
            let cart=await this.cartModel.findOne({user: query.userId, isOrderLinked: false},'cart');
            if(cart && cart.cart && cart.cart.length){
    ...
    public async searchProduct(search_key: string,query): Promise<CommonResponseModel> {
        let products = await this.productModel.find( { title: { $regex:search_key,$options: 'i' },status:1},'title filePath imageUrl isDealAvailable delaPercent category variant averageRating');
        if(query && query.userId && products && products.length){
            let cart=await this.cartModel.findOne({user: query.userId, isOrderLinked: false},'cart');
            if(cart && cart.cart && cart.cart.length){
                products=await this.ProductDataCustomize(cart,products)
    ...
    public async getProductsByCategory(categoryId: string,query:any): Promise<CommonResponseModel> {
        let products = await this.productModel.find({category: categoryId,status:1},'title imageUrl filePath category isDealAvailable delaPercent variant averageRating').sort({createdAt:-1});
    ...
    public async getProductsBySubCategory(subCategoryId: string,query:any): Promise<CommonResponseModel> {
        let products = await this.productModel.find({subcategory: subCategoryId,status:1},'title imageUrl filePath category subcategory isDealAvailable delaPercent variant averageRating').sort({createdAt:-1});
        if(query && query.userId && products && products.length){
            let cart=await this.cartModel.findOne({user: query.userId, isOrderLinked: false},'cart');
            if(cart && cart.cart && cart.cart.length){
                products=await this.ProductDataCustomize(cart,products)
