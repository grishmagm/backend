...
export class OrderService {
    ...
    public async getTaxRelatedInformation(cartInfo: CartDTO): Promise<CommonResponseModel> {
    ...
    public async saveOderData(orderdDATA: NewOderModel): Promise<CommonResponseModel> {
       ...
       // await this.productService.productStockManagement(orderdDATA)
    ...
    // public async testSeq():Promise<CommonResponseModel>{
    //     let test=await this.sequenceModel.findOneAndUpdate({"sequenceType": "order"},{$inc: { sequenceNo: 1}},{new:true})
    //     return{
    //         response_code:HttpStatus.OK,
    //         response_data:test.sequenceNo
    //     }
    // }
}
